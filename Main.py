...
class MagicMirrorApp(tk.Tk):
    def __init__(self):
        ...
        if not self.cap.isOpened():
            # This will be displayed via the widget manager now
            pass
    ...
    def load_config(self):
        ...
        if os.path.exists(CONFIG_FILE):
            ...
            if "widget_positions" not in self.config:
                self.config["widget_positions"] = default_config["widget_positions"]
            else:
    ...
    def save_settings_from_panel(self):
        ...
        # Trigger updates in widgets
        self.widget_manager.widgets['weather']._update_text()
        self.widget_manager.widgets['news']._update_text()
    ...
    def get_widget_bbox(self, widget_name, win_width, win_height):
        ...
        if not widget or not widget.text: return None
        
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_scale = widget.params['scale']
        thickness = widget.params['thick']
    ...
    def toggle_fullscreen(self, event=None):
        ...
        if is_fullscreen:
            # Exiting fullscreen, set a fixed size
